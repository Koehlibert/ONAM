...
plot_labels_inter <-
  apply(expand.grid(LETTERS[1:9], 1:2) %>% arrange(Var1),
        1, function(x) paste(x, collapse = ""))
plot_theme_main <- theme(plot.caption = element_text(hjust = 0),
...
get_average_data_inter <- function(data)
{
  ...
  plot_data_inter <- plot_data_inter[which(!is.na(plot_data_inter$Prediction)),]
...
{
  return(scale_fill_gradientn(colors =
                                 customColors(n = 100),
                               values =
